...
function init() {
    renderCameraList();
    renderMarkers();
    const countTip = document.getElementById('camera-count-tip');
    if (countTip) countTip.textContent = `共 ${cameras.length} 个镜头可查看`;
    updateSystemTime();
...
function switchCamera(id) {
    ...
    loader.querySelector('span').textContent = "正在连接镜头...";
...
function handleZoom(direction) {
    ...
    if (newScale >= PTZ_CONFIG.minScale && newScale <= PTZ_CONFIG.maxScale) {
        ...
        validatePosition(); 
        applyTransform();
        showFeedback(direction > 0 ? "镜头拉近" : "镜头拉远");
    }
}
...
function validatePosition() {
    ...
    const width = videoEl.clientWidth || 800; 
    const height = videoEl.clientHeight || 450;
...
function showFeedback(text) {
    ...
    if (loader.style.display === 'flex' && text.includes('连接')) return;
